...
export default async function HomePage() {
  let domains: Domain[] = [];
  try {
    domains = await listDomains({ publishedOnly: true });
  } catch {
  ...
  await Promise.all(
    domains.map(async (d) => {
      try {
        leaderboards[d.id] = await getLeaderboard(d.id);
      } catch {
        leaderboards[d.id] = [];
      }
    }),
        ...
        <header className="mb-10 flex items-end justify-between gap-6 border-b border-line pb-6">
          <div>
            ...
            <p className="mt-3 max-w-prose text-ink-2 text-[15px] leading-relaxed">
              Each entry is bound to a versioned vow — observation, action,
              and reward shapes you can read at a glance. Run a model, climb
        ...
        {domains.length === 0 ? (
          <div className="py-32 text-center">
            ...
            <p className="mt-3 text-sm text-ink-3 max-w-sm mx-auto">
              ...
              <Link href="/developer" className="text-ink hover:text-leaf-deep transition-colors underline underline-offset-2">
                developer page
              </Link>{" "}
    ...
    <section className="hero">
      <div className="hero-wrap">
        ...
        <div className="hero-cta">
          ...
          <Link
            href="/showcase"
            className="text-sm text-ink hover:text-leaf-deep transition-colors inline-flex items-center gap-2"
          >
        ...
        <div className="hero-meta-bottom">
          <span>
            {exhibitsLive > 0 ? `${exhibitsLive} exhibits live` : "in development"}{" "}
            · 38 contributors this month
          </span>
          <span className="hero-etym-line">
            from <em>bench</em> (a place of measure) and <em>anything</em> (any
...
// ── Process — three-step service description ───────────────────────
